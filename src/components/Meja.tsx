...
const Meja=()=> {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('http://localhost:8080/api/meja');
  ...
  const handleAdd = async () => {
    try {
      const response = await axios.post('http://localhost:8080/api/meja', {
        nomor_meja: newMeja,
      });

      setNewMeja('');
      setMeja((prevItem) => [...prevItem, response.data.data]);
    } catch (error) {
  ...
  const handleEdit = (item:any) => {
    setEditMeja(item);
    setNewMeja(item.nomor_meja);
  };

  const handleUpdate = async () => {
    try {
      const updateditem = await axios.put(
        `http://localhost:8080/api/meja/${editMeja.id}`,
        {
          nomor_meja: newMeja,
        }
      );
  ...
  const handleDelete = async (id:any) => {
    try {
      await axios.delete(`http://localhost:8080/api/meja/${id}`);
      setMeja((prevItem) => prevItem.filter((item) => item.id !== id));
    } catch (error) {
    ...
    <Box>
      ...
      <Grid item md={12}>
          <Paper sx={{width:'100%',textAlign:'center', padding:'50px', margin:'auto', borderRadius:'15px', background:'#209F84', color:'white'}}> 
              <Typography variant='h3' sx={{ fontWeight:'bold' }}>Meja</Typography>
          </Paper>
      </Grid>
      <Grid item md={6}>
      <Box>
      ...
      <Grid item md={(6)}>
        {meja.map((item:any, index) => (
         ...
         >  
              <Box>
                <Typography>{item.nomor_meja}</Typography>
              </Box>
            <Box>
                <Button
